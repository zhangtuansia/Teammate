# Code of Conduct

Teammate follows the [Contributor Covenant v2.1](https://www.contributor-covenant.org/version/2/1/code_of_conduct/). The full text and translations live at the link above.

## In short

- Be welcoming and respectful. Disagreements happen — keep them about ideas, not people.
- No harassment, personal attacks, or discriminatory language.
- Assume good intent. Ask before assuming the worst.
- If you're unsure whether something is okay to say, it probably isn't worth saying.

## Scope

This applies to all project spaces — issues, pull requests, discussions, code reviews — and to anyone representing the project in public.

## Reporting

If you experience or witness behavior that violates this code, contact the maintainers privately through the repository security contact. Reports are kept confidential and will be reviewed as quickly as possible.
